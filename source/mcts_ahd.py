...
class MCTS_AHD:
    ...
    def expand_batch(self, mcts, cur_node, nodes_set, operators):
        """
        并行执行多个操作符，批量生成offsprings
        
        Args:
            mcts: MCTS树
            cur_node: 当前节点
            nodes_set: 当前种群
            operators: 操作符列表（考虑权重重复）
        ...
        # 准备pop参数（根据不同操作符类型）
        # 对于s1操作符，需要特殊处理
        s1_operators = [op for op in operators if op == 's1']
        other_operators = [op for op in operators if op != 's1']
        
        successful_offsprings = []
        
        # 先处理s1操作符（如果有）
        for op in s1_operators:
            if op == 's1':
                path_set = []
                now = copy.deepcopy(cur_node)
                while now.code != "Root":
                ...
                # 使用串行版本的evolve_algorithm处理s1
                self.eval_times, offspring = self.interface_ec.evolve_algorithm(
                    self.eval_times, path_set, cur_node.raw_info, cur_node.children_info, op
                )
                if offspring is not None:
                    successful_offsprings.append((op, offspring))
        
        # 并行处理其他操作符
        if other_operators:
            # 对于非s1操作符，使用相同的pop参数
            self.eval_times, offsprings_list = self.interface_ec.evolve_algorithms_parallel(
                self.eval_times, nodes_set, cur_node.raw_info, cur_node.children_info, other_operators
            )
            
            # 将并行生成的结果添加到successful_offsprings
            for offspring in offsprings_list:
                # 确定这个offspring是由哪个操作符生成的
                # 由于并行执行，我们需要一个映射关系
                # 简化处理：假设顺序对应
                if offsprings_list:
                    # 实际上，evolve_algorithms_parallel返回的offsprings_list
                    # 已经包含了操作符信息（在调试信息中）
                    successful_offsprings.append(('parallel', offspring))
        
        # 处理所有成功的offsprings
        for op, offspring in successful_offsprings:
            if offspring is None:
                continue
                
            if op != 'e1' and op != 'parallel':  # parallel表示来自并行执行
                print(
                    f"Action: {op}, Father Obj: {cur_node.raw_info['objective']}, Now Obj: {offspring['objective']}, Depth: {cur_node.depth + 1}")
            elif op == 'e1':
                if self.interface_ec.check_duplicate_obj(mcts.root.children_info, offspring['objective']):
                    print(f"Duplicated e1, no action, Father is Root, Abandon Obj: {offspring['objective']}")
                    continue
                else:
                    print(f"Action: {op}, Father is Root, Now Obj: {offspring['objective']}")
            else:  # parallel
                print(f"Action: parallel, Father Obj: {cur_node.raw_info['objective']}, Now Obj: {offspring['objective']}, Depth: {cur_node.depth + 1}")
            
            if offspring['objective'] != float('inf'):
                self.add2pop(nodes_set, offspring)  # Check duplication, and add the new offspring
                size_act = min(len(nodes_set), self.pop_size)
                nodes_set = self.manage.population_management(nodes_set, size_act)
                nownode = MCTSNode(offspring['algorithm'], offspring['code'], offspring['objective'],
                                   parent=cur_node, depth=cur_node.depth + 1,
                                   visit=1, Q=-1 * offspring['objective'], raw_info=offspring)
                if op == 'e1':
                    nownode.subtree.append(nownode)
                cur_node.add_child(nownode)
    ...
    def run(self):
        ...
        while self.eval_times < self.fe_max:
            ...
            for i in range(n_op):
                ...
                # 根据权重重复操作符
                for j in range(op_w):
                    operators_batch.append(op)
            
            print(f"Iter: {self.eval_times}/{self.fe_max} | Batch size: {len(operators_batch)} operators")
            
            # 使用批量扩展方法
            nodes_set = self.expand_batch(mcts, cur_node, nodes_set, operators_batch)
