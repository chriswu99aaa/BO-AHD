...
class BOInterface():
    ...
    def generate_heuristic_by_action(self, action: str, max_retries: int = 5):
        """
        ...
        """
        return self._generate_with_retry(action, max_retries)
    
    def _generate_with_retry(self, action: str, max_retries: int = 5) -> Tuple[str, str]:
        """
        Generate heuristic with retry logic and exponential backoff.
        ...
        for attempt in range(max_retries):
            try:
                ...
                logging.debug(f"Successfully generated heuristic for action {action} on attempt {attempt + 1}")
                return code, algorithm
                
            except Exception as e:
                last_error = str(e)
                logging.warning(f"Attempt {attempt + 1}/{max_retries} failed for action {action}: {last_error}")
        ...
        # All retries failed
        error_msg = f"Failed to generate heuristic for action {action} after {max_retries} attempts. Last error: {last_error}"
