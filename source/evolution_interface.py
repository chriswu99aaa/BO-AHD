...
class InterfaceEC():
    ...
    def evolve_algorithms_parallel(self, eval_times, pop, node, brother_node, operators):
        """
        并行执行多个操作符，生成多个offspring
        ...
        Args:
            ...
            node: 当前节点
            brother_node: 兄弟节点信息
            operators: 操作符列表，每个操作符可能重复多次（根据权重）
            
        Returns:
            eval_times: 更新后的评估次数
            offsprings_list: 成功生成的offspring列表
        """
        if not operators:
            return eval_times, []
        
        successful_offsprings = []
        total_eval_increment = 0
        
        # 使用线程池并行执行
        with concurrent.futures.ThreadPoolExecutor(max_workers=self.n_p) as executor:
            # 准备任务：每个操作符生成一个offspring
            future_to_operator = {}
            for operator in operators:
                # 提交任务
                future = executor.submit(
                    self._evolve_single_operator,
                    pop, node, brother_node, operator
                )
                future_to_operator[future] = operator
            
            # 收集结果
            for future in concurrent.futures.as_completed(future_to_operator, timeout=self.timeout * len(operators)):
                operator = future_to_operator[future]
                try:
                    offspring = future.result(timeout=self.timeout)
                    if offspring is not None:
                        successful_offsprings.append(offspring)
                        total_eval_increment += 1  # 每个成功的offspring消耗一次评估
                        if self.debug:
                            print(f"Parallel operator {operator} succeeded, objective: {offspring['objective']}")
                    else:
                        if self.debug:
                            print(f"Parallel operator {operator} failed after attempts")
                except concurrent.futures.TimeoutError:
                    if self.debug:
                        print(f"Parallel operator {operator} timed out after {self.timeout} seconds")
                except Exception as e:
                    if self.debug:
                        print(f"Parallel operator {operator} failed with exception: {e}")
        
        # 更新评估次数
        eval_times += total_eval_increment
        return eval_times, successful_offsprings

    def _evolve_single_operator(self, pop, node, brother_node, operator):
        """
        单个操作符的生成和评估，用于并行执行
        ...
        Returns:
            offspring: 成功生成的offspring，失败返回None
        """
        for attempt in range(3):  # 每个操作符最多尝试3次
            try:
                _, offspring = self.get_offspring(pop, operator, father=node)
                objs = self.interface_eval.batch_evaluate([offspring['code']], 0)
                if objs == 'timeout':
                ...
                if self.debug:
                    print(f"Operator {operator} attempt {attempt+1} failed: {e}")
                # 等待一小段时间再重试（避免同时重试）
                time.sleep(0.1)
        return None