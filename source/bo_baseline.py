...
class BayesianOptimizer_Baseline(BayesianOptimizer_Local):
    ...
    def create_feature_vector(self, code, action):
        ...
        feature_vector = np.concatenate([code_embedding_reduced, action_onehot])
    ...
    def initialize(self):
        ...
        for attempt in range(max_retries):
            ...
            print(f"Initial tour length: {train_length}")
    ...
    def fit(self):
        ...
        if np.isnan(X_arr).any() or np.isinf(X_arr).any():
            print("ERROR: NaN or inf detected in self.X")
            print("self.X:", X_arr)
            raise ValueError("NaN or inf detected in self.X")
        if np.isnan(Y_arr).any() or np.isinf(Y_arr).any():
            print("ERROR: NaN or inf detected in self.Y")
            print("self.Y:", Y_arr)
            raise ValueError("NaN or inf detected in self.Y")
            ...
            for threshold in threshold_vals:
                try:
                    acq_func = qPredictiveEntropySearch(
                        ...
                        maximize=False,
                        X_pending=None,
                        max_ep_iterations=50,  # previous 50
                        ep_jitter=jitter,
                        test_jitter=jitter,
                        threshold=threshold
                    )
    ...
    def check_similar(self, fv, thr=0.98):
        ...
        X = np.vstack(self.X).astype(np.float64)   # 历史特征
        fv = fv.astype(np.float64)

        # 1) history feature vector
        # E_code = X[:, :64]
        # v_code = fv[:64]
        E_code = X[:, :70] #64pca 6 action one-hot encoding
        v_code = fv[:70]
        denom = (np.linalg.norm(E_code, axis=1) * (np.linalg.norm(v_code) + 1e-12) + 1e-12)
        cos = (E_code @ v_code) / denom            # cosine similarity
        score = cos
        return score.max() > thr
    ...
    def run(self):
        ...
        for iteration in range(self.count, self.n_iters):
            ...
            sampled_parents = random.choice(self.population) #single node
            children = self.expand(sampled_parents)
            if children:
            ...
            selected_codes = [child.code for child in all_child_nodes]
            try:
                train_lengths = self.problem.batch_evaluate(selected_codes, 0)
                print(f"Batch evaluated {len(selected_codes)} children")
                
                # Assign evaluation results to children and filter valid ones
                valid_children = []
                for child, train_length in zip(all_child_nodes, train_lengths):
                    child.train_length = train_length
                    print(f"Child tour length: {train_length}")
            ...
            best_data = {
                ...
                "code": best_node.code,
                "objective": best_node.train_length,
        ...
        if self.problem.obj_type == 'min':
            ...
            best_node = max(self.population, key=lambda x: x.train_length if x.train_length is not None else float('-inf'))
        print(f"\n=== Optimization Complete ===")
        print(f"Best Performance: {best_node.train_length}")
    ...
    def load_population_from_file(self, filename):
        ...
        # self.initial_epsilon = 1.0 * 0.99 ** self.count  # Reset epsilon based on count
