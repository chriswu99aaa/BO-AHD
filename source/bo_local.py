...
from source.parallel_executor import ParallelActionExecutor, GenerationResult
...
class BayesianOptimizer_Local():
    def __init__(self,  paras, problem, **kwargs):
        ...
        # Parallel executor settings (added for shared executor)
        self.max_workers = 6  # 对应 6 个 action types
        self.timeout_per_action = 300  # 每个 action 的超时时间
        self.executor = None  # 共享的 ParallelActionExecutor 实例
    ...
    def embed_code(self, code):
        ...
        try:
            ...
            # 检查 embedding 中是否有 NaN 或 Inf
            if np.isnan(embedding_array).any() or np.isinf(embedding_array).any():
                print(f"Warning: Embedding contains NaN or Inf values, replacing with zeros")
    ...
    def initialize(self):
        ...
        # Retry logic for initial heuristic generation if NaN or Inf detected
        max_retries = 10
        for attempt in range(max_retries):
            try:
                code, description = self.bo_interface.generate_heuristic_by_action(action="i1")
                print(f"Initial response: {code[:200]}..., {description}")
            except Exception as e:
                print(f"Failed to generate initial heuristic on attempt {attempt+1}: {e}")
                continue
            ...
            # Check for NaN/Inf in feature vector and train_length
            feature_valid = not (np.isnan(feature_vector).any() or np.isinf(feature_vector).any())
            length_valid = isinstance(train_length, (int, float)) and np.isfinite(train_length)
            
            if feature_valid and length_valid:
                # Create root node
                ...
                print(f"Initialization attempt {attempt+1}: Detected invalid value.")
                print(f"  Feature valid: {feature_valid}, length valid: {length_valid}")
                print(f"  Train length: {train_length}, is finite: {np.isfinite(train_length) if isinstance(train_length, (int, float)) else 'N/A'}")
                print(f"  Feature vector NaN/Inf: {np.isnan(feature_vector).any() or np.isinf(feature_vector).any()}")
                print(f"  Retrying...")
                # Small delay before retry
                import time
                time.sleep(1)
        else:
            raise RuntimeError(f"Failed to generate valid initial heuristic after {max_retries} attempts.")
    ...
    def fit(self):
        ...
        if not valid_indices:
            raise ValueError("No valid (finite) observations in self.Y. Contains only NaN/Inf values.")
        
        # 使用有效数据构建训练集
        X_arr = np.vstack([self.X[i] for i in valid_indices])
        Y_arr = np.array([self.Y[i] for i in valid_indices])
        
        # 检查 X 中是否有 NaN
        if np.isnan(X_arr).any():
            raise ValueError("NaN values found in feature vectors (self.X).")
        
        print(f"Fitting GP with {len(valid_indices)} valid observations (filtered out {len(self.Y) - len(valid_indices)} invalid).")
    ...
    def expand(self, node: HeuristicNode):
        """
        Expand the node by generating new heuristics for each action using shared parallel executor.
        This version only generates nodes without evaluating them.
        """
        max_retries = 5
        new_nodes = []

        # Filter out i1 action (only for initialization)
        expansion_actions = [action for action in self.action_types if action != 'i1']
        if not expansion_actions:
            return new_nodes
        
        print(f"Starting parallel expansion for actions: {expansion_actions}")
        
        # Use shared parallel executor
        try:
            if self.executor is None:
                self.executor = ParallelActionExecutor(
                    self.bo_interface,
            ...
            results = self.executor.execute_actions(expansion_actions, node, max_retries=max_retries)
            
            # Process results
            for result in results:
                if not result.success:
                    print(f"Action {result.action} failed: {result.error}")
                    continue
                
                if not result.code or not result.algorithm:
                    print(f"Action {result.action} returned empty code or algorithm")
                    continue
                
                # Check for duplicate code
                if self.check_duplicate_code(self.population, result.code):
                    print(f"Duplicate code detected for action {result.action}, skipping.")
                    continue
                
                # Create feature vector
                feature_vector = self.create_feature_vector(result.code, result.action)
                
                # Check for similar feature vectors (skip for mutation actions)
                if self.check_similar(feature_vector) and result.action not in ['m1', 'm2']:
                    print(f"Similar feature vector detected for action {result.action}, skipping child.")
                    continue
                ...
                print(f"Successfully generated heuristic for action {result.action} (unevaluated, attempts: {result.attempts})")
            
            # Note: executor is NOT shutdown here - it will be reused for subsequent expansions
            
        except Exception as e:
            print(f"Parallel expansion failed: {e}")
            traceback.print_exc()
            # Fallback to sequential expansion
            print("Falling back to sequential expansion...")
            return self._expand_sequential_fallback(node, expansion_actions, max_retries)
        
        print(f"Parallel expansion completed: {len(new_nodes)} new nodes generated")
        return new_nodes


    def _expand_sequential_fallback(self, node: HeuristicNode, actions: list, max_retries: int):
        """
        Fallback sequential expansion method if parallel execution fails.
        """
        new_nodes = []
        
        for action in actions:
            print(f"Sequential fallback: Generating heuristic for action: {action}")
    ...
    def select_parents_ucb(self, model, beta:float=2.0, turn_on = False):
        """
        Select parents using Upper Confidence Bound (UCB) strategy.
        turn_on: whether to use UCB or random selection
        """
        if len(self.X) == 0:
            return []
        ...
        if turn_on == True:
            ...
            idx = np.random.choice(len(self.population), size=k, replace=False).tolist()
            print(f"Random pick  -> nodes: {[self.population[i] for i in idx]}")
            return [self.population[i] for i in idx]
    ...
    def run(self):
        ...
        for iteration in range(self.count, self.n_iters):
            ...
            for selected_child in valid_children:
                # 在添加之前再次检查特征向量和训练长度是否有 NaN/Inf
                if np.isnan(selected_child.feature_vector).any() or np.isinf(selected_child.feature_vector).any():
                    print(f"Warning: Skipping child due to NaN/Inf in feature vector")
                    continue
                
                if not (isinstance(selected_child.train_length, (int, float)) and np.isfinite(selected_child.train_length)):
                    print(f"Warning: Skipping child due to invalid train_length: {selected_child.train_length}")
                    continue
                ...
                print(f"Added child to training data: train_length={selected_child.train_length}, feature_vector_shape={selected_child.feature_vector.shape}")
        ...
        # Cleanup: shutdown the shared executor when run completes
        if self.executor:
    ...
    def load_population_from_file(self, filename):
        ...
        for item in data:
            ...
            # 检查 objective 是否为有效数值
            if objective is None or (isinstance(objective, (int, float)) and not np.isfinite(objective)):
                print(f"Warning: Skipping invalid objective value: {objective}")
                continue
            ...
            if np.isnan(feature_vector).any():
                print(f"Warning: Skipping node due to NaN in feature vector")
                continue
        ...
        for idx, item in enumerate(data):
            ...
            if parent_idx is not None and parent_idx < len(self.population) and idx < len(self.population):
        ...
        print(f"Loaded {valid_count} valid individuals from file (skipped {len(data) - valid_count} invalid), starting iteration: {self.count}")
