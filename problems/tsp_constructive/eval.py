...
def eval_heuristic(node_positions: np.ndarray) -> float:
    ...
    for _ in range(problem_size - 1):
        ...
        if next_node in unvisited:
            ...
            # If the fallback also fails, we have a problem.
            # This should only happen if unvisited is empty, but the loop structure prevents that.
            if len(unvisited) > 0:
                raise KeyError(f"Node {next_node} is still invalid after fallback.")
