...
class Prompts:
    def __init__(self, problem_cfg, root_dir: str):
        ...
        elif self.prompt_func_name.startswith('choose_action'):
            self.prompt_func_outputs = ['action']
        else:
...
class Problem:
    def __init__(self, cfg, root_dir):
        ...
        self.problems_path = f"{self.root_dir}/problems/{self.problem}" #used when running eval.py
        self.output_file_bo = f"{self.root_dir}/problems/{self.problem}/gpt_bo.py"
    ...
    def batch_evaluate(self, codes: list[str], iteration: int) -> str | list[Any]:
        ...
        for response_id in range(len(population)):
            ...
            try:
                ...
                return 'timeout'
            # finally:
            #     if inner_run and inner_run.poll() is None:
            #         inner_run.kill()
            ...
            if traceback_msg == '':  # If execution has no error
                ...
                print(f"Traceback for response_id {response_id}: {traceback_msg}")
    ...
    def batch_evaluate_multi(self, codes: list[str], iteration: int) -> list[dict]:
        """
        Evaluate population on both train and val sets, returning both results for each code.
        Returns: List of dicts: [{"train": float, "val": float, "gap": float, "success": bool, "traceback": str}]
        """
        self.iteration = iteration
        population = [self.response_to_individual(resp, index) for index, resp in enumerate(codes)]
        results = []
        for response_id in range(len(population)):
            code = population[response_id]["code"]
            result = {"train": None, "val": None, "gap": None, "success": False, "traceback": ""}
            # Evaluate on train
            train_proc = None
            try:
                runid = hash(code)
                with open(self.output_file, 'w', encoding='utf-8') as file:
                    file.writelines(code + '\n')
                # Train
                train_stdout = f'./evaluations/problem_eval{runid}_train_stdout.txt'
                with open(train_stdout, 'w') as f:
                    file_path = f'{self.root_dir}/problems/{self.problem}/eval.py' if self.problem_type != "black_box" else f'{self.root_dir}/problems/{self.problem}/eval_black_box.py'
                    train_proc = subprocess.Popen(
                        ['python', '-u', file_path, f'{self.problem_size}', self.root_dir, "train"], stdout=f, stderr=f)
                block_until_running(train_stdout, log_status=True)
                train_proc.communicate(timeout=self.config.timeout)
                with open(train_stdout, 'r') as f:
                    train_out = f.read()
                train_traceback = filter_traceback(train_out)
                if train_traceback == '':
                    train_obj = float(train_out.split('\n')[-2])
                    train_obj = -train_obj if self.obj_type == "max" else train_obj
                    result["train"] = train_obj
                else:
                    result["traceback"] = f"train: {train_traceback}"
                    results.append(result)
                    continue
            except Exception as e:
                result["traceback"] = f"train: {str(e)}"
                results.append(result)
                continue
            finally:
            ...
            val_proc = None
            try:
                val_stdout = f'./evaluations/problem_eval{runid}_val_stdout.txt'
                with open(val_stdout, 'w') as f:
                    file_path = f'{self.root_dir}/problems/{self.problem}/eval.py' if self.problem_type != "black_box" else f'{self.root_dir}/problems/{self.problem}/eval_black_box.py'
                    val_proc = subprocess.Popen(
                        ['python', '-u', file_path, f'{self.problem_size}', self.root_dir, "val"], stdout=f, stderr=f)
                block_until_running(val_stdout, log_status=True)
                val_proc.communicate(timeout=self.config.timeout)
                with open(val_stdout, 'r') as f:
                    val_out = f.read()
                val_traceback = filter_traceback(val_out)
                if val_traceback == '':
                    val_obj = float(val_out.split('\n')[-2])
                    val_obj = -val_obj if self.obj_type == "max" else val_obj
                    result["val"] = val_obj
                else:
                    result["traceback"] += f" val: {val_traceback}"
            except Exception as e:
                result["traceback"] += f" val: {str(e)}"
            # finally:
            #     if val_proc and val_proc.poll() is None:
            #         val_proc.kill()
            ...
            if "val" not in result:
                ...
                continue
            # Compute gap
            if result["train"] is not None and result["val"] is not None:
                result["gap"] = abs(result["train"] - result["val"])
                result["success"] = True
            results.append(result)
        return results
